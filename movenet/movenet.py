...
video_path = 'video/squat_2.mp4'
cap = cv2.VideoCapture(0)
num_frame = 0
output_images= []

while True:
    # bar = display(progress(0, num_frame-1), display_id=True)
    ret, image = cap.read()
    image = tf.convert_to_tensor(image)
    image_height, image_width, _ = image.shape
    crop_region = init_crop_region(image_height, image_width)
    keypoints_with_scores = run_inference(movenet, image, crop_region, crop_size=[input_size, input_size])
    new_image = (draw_prediction_on_image(image.numpy().astype(np.int32),keypoints_with_scores, crop_region=None,close_figure=True, output_image_height=300))
    output_images.append(new_image)
    crop_region = determine_crop_region(keypoints_with_scores, image_height, image_width)
    cv2.imshow('movenet', new_image)
    # num_frame += 1
