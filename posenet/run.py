...
def squat_down(angle, angles_arr, squat_knee_angle, left_knee_angle, right_knee_angle, left_hip_gap, right_hip_gap, knee_gap):
    ...
    if len(angles_arr)>10:
        if mean(angles_arr[-10:-5]) < mean(angles_arr[-5:]) and angle > angles_arr[0] + 5 :
            ...
            knee_angle_standard = squat_knee_angle * 1.25
            if left_knee_angle > knee_angle_standard and right_knee_angle > knee_angle_standard: 
                test="Lower"
                color = (0,0,250)

            elif (squat_knee_angle <= left_knee_angle < knee_angle_standard or 
                    squat_knee_angle <= right_knee_angle < knee_angle_standard) or \
        ...
        if mean(angles_arr[-10:-5]) > mean(angles_arr[-5:]):
            ...
            if np.median(angles_arr[:5])*0.95 <= angle <= np.median(angles_arr[:5]) * 1.05: #완전히 "섰다"의 인식이 쫌 여유가 있는 듯 
                ...
                if count_flag:
                    ...
                    threading.Thread(target=speak, args=(str(squat_count)+' 번 좋습니다',)).start()
                    # threading.Thread(target=speak, args=("좋다",)).start()
...
class RestWindow(QDialog, rest_form):
    def __init__(self):
        ...
        self.duration = CUSTOM_SQUAT_REST
...
class SettingWindow(QDialog, setting_form):
    def __init__(self):
        ...
        self.restsText.setValidator(QtGui.QIntValidator())

        self.countText.setText(str(CUSTOM_SQUAT_COUNT))
        self.setsText.setText(str(CUSTOM_SQUAT_SET))
        self.restsText.setText(str(CUSTOM_SQUAT_REST))
    ...
    def my_accept(self):
        ...
        CUSTOM_SQUAT_REST = int(self.restsText.text()) 
...
class MyWindow(QMainWindow, form_class):
    ...
    def web_btn_clicked(self):
        webbrowser.open('http://52.79.176.139:5001')
            ...
            if res and running: #영상이 인식이 된다면
                ...
                if real_start==False:#사람이 인식되지 않았을 때
                    if startcount<=0:#두번 반복을 위한
                        ...
                        now_tape = "l"
                        cam_start = 2  #캠 인식 시작
                        threading.Thread(target=speak, args=('준비하세요',)).start()
                ...
                if cam_start==2:
                    ...
                    if real_start == False:
                        ...
                        if R_shoulder & R_elbow & R_pelvis & R_knee & L_shoulder & L_elbow & L_pelvis & L_knee:
                            ...
                            if time_count > 3:
                                real_start = True
                                threading.Thread(target=speak, args=('시작하세요',)).start()
