...
def squat_down(ratio, grad, squat_knee_angle, left_knee_angle, right_knee_angle, left_side_angle, right_side_angle):
    global color
    global test
    squat_side_angle = 90
    # 스쿼트 단계: 내려감
    if ratio <=1.5: # 발목과 코의 거리 비율이 줄어들 때
        if abs(grad) <= 0.1 : # 
                ...
                if ret:
                    ...
                    if real_start:
                        if len(ratio_arr)>20: 
                            ...
                            grad = sum(ratio_arr[-20:-11]) - sum(ratio_arr[-10:-1]) # 평균으로 할 경우 값이 너무 작아 기울기 판별이 어려워 sum으로 대체 ################################
                            grad_arr.append(grad)

                            # 자세 판별
                            squat_knee_angle = 90

                            squat_down(ratio, grad, squat_knee_angle, angle_save["L knee"],  angle_save["R knee"], angle_save["L hip"], angle_save["R hip"])
