...
engine = pyttsx3.init() # engine initialize
...
def squat_down(angle, angles_arr, squat_knee_angle, left_knee_angle, right_knee_angle, left_hip_gap, right_hip_gap, knee_gap):
    ...
    global count_flag
    global squat_count
    global video_player# video play trigger
    global squat_set
    global rest_flag
    global set_flag
    global knee_flag
    ...
    if len(angles_arr)>10:
        if mean(angles_arr[-10:-5]) < mean(angles_arr[-5:]) and angle > angles_arr[0] + 5 :
            ...
            if left_knee_angle > squat_knee_angle * 1.2 and right_knee_angle > squat_knee_angle * 1.2: 
                ...
                test="Good"
                video_player=0 #ready playing video
                count_flag = True
                color = (255,0,0)
        ...
        if mean(angles_arr[-10:-5]) > mean(angles_arr[-5:]):
            ...
            # hold
            if np.median(angles_arr[:5])*0.95 <= angle <= np.median(angles_arr[:5]) * 1.05:
...
def angle_flag(now_angle,now_mins,now_maxs,now_flag,errocounter,joint,wrongtext):
...
angle_dict = {
    'R hip':(12,6,14),  #right side
    'L hip':(11,5,13),  #left side
    'R knee':(14,16,12),    #right knee
    'L knee':(13,15,11),     #left knee
...
def angle_cal(A,B,C):# calcuating angle in A
